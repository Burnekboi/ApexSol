...
Click Transfer Now to proceed.`,
  ...
  if (['wallet_status_setup','wallet_status_post','wallet_status_trade'].includes(data)) {
    ...
    try {
      ...
      session.mainWallet.lastBalance = balance;
    } catch {}
...
`;
    ...
    const menuMap = {
      ...
      wallet_status_trade: panels.actionMenu(session, session.mainWallet?.lastBalance?.toFixed(4) || "0.0000")
    };
  ...
  if (['bot_balances', 'bot_balances_post'].includes(data)) {
    if (!session.buyers?.length) {
      return editText('❌ No buyer wallets.', {
        ...
        ...(data === 'bot_balances'
          ? panels.postBuyerMenu()
          : panels.actionMenu(session, session.mainWallet?.lastBalance?.toFixed(4) || "0.0000"))
      });
    }
  ...
  if (['auto_transfer', 'paid_transfer'].includes(data)) {
    ...
    const menuOnFail = () => (wasAddBot ? panels.actionMenu(session, session.mainWallet?.lastBalance?.toFixed(4) || "0.0000") : panels.postBuyerMenu());
